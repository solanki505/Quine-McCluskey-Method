...
bitSize = int(input("Enter number of variables: "))
def decimal_to_binary(minterm, bits=bitSize):
    # Convert the decimal number to binary and remove the '0b' prefix
    return format(int(minterm), f'0{bits}b')
minterms = input("Enter minterms space separated : ").split()
dontcare = input("Enter dontcares: ").split()
print("Given minterms are ")
print(minterms)
print("Don't care -")
...
for i in minterms_dontcare:
    ...
    if j not in visited:
        k,=j
        unvisited.append(((k,),str(decimal_to_binary(int(k)))))
def minimization():
    global max_space
    dict1.clear()
    dict1.update(dict2)
